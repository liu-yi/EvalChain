...
function mod_sqrt(a, p) {
    ...
    for (var b = BigInteger('2'); b.compareTo(p) < 0; b = b.add(BigInteger('1'))) {
        if (jacobi(b.multiply(b).subtract(BigInteger('4').multiply(a)), p).equals(BigInteger('-1'))) {
            f = [a, b.negate(), 1];
            var ff = polynomial_exp_mod([BigInteger('0'), BigInteger('1')], p.add(BigInteger('1')).divide(BigInteger('2')), f, p);
...
function H2(y) {
    return map_to_curve(hashToInt(y));
}
...
function H1(y, link, message, z_1, z_2) {
    ...
    temp_list = temp_list.concat(y.map(
        (item) => {
            // return new BN(item.toString()).toString(16, 64);
    ...
    temp_list = temp_list.concat(
        [(new BN(link.toString().split(',')[0].substring(1)).toString(16, 64)) + (new BN(link.toString().split(',')[1].substring(1)).toString(16, 64))]
    );
    temp_list = temp_list.concat([new BN(message.toString()).toString(16, 64)]);
    temp_list = temp_list.concat(
        [(new BN(z_1.toString().split(',')[0].substring(1)).toString(16, 64)) + (new BN(z_1.toString().split(',')[1].substring(1)).toString(16, 64))]
    );
    temp_list = temp_list.concat(
        [(new BN(z_2.toString().split(',')[0].substring(1)).toString(16, 64)) + (new BN(z_2.toString().split(',')[1].substring(1)).toString(16, 64))]
    );
...
function hashToInt(y) {
    ...
    return BigInteger.fromHex(web3Utils.keccak256('0x' + temp_y.join('')).toString().substring(2));
}


//key_idx is not big integer!!
function ring_signature(siging_key, key_idx, M, y) {
    var G = ecparams.G;
...
function main() {
    ...
    var signature = ring_signature(x[my], my, message, y);
    console.log(verifyRingSignature(message, y, signature[0], signature[1], signature[2]));
